...
from PySide6.QtGui import (QColor, QBrush, QPainter, QPainterPath, QPen,
                           QPolygonF, QRadialGradient, QImage, QLinearGradient, QPixmap)
from PySide6.QtWidgets import (QApplication, QGraphicsItem, QGraphicsScene,
                               QGraphicsView, QGraphicsPixmapItem, QStyle, QWidget)
...
class Edge(QGraphicsItem):
    ...
    def adjust(self):
        ...
        edge_offset = QPointF((line.dx() * 15) / length, (line.dy() * 15) / length)
...
class Node(QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        QGraphicsPixmapItem(QPixmap("images/ghost.png"))
        ...
        painter.drawRect(-10, -10, 20, 20)
        # # painter.drawImage(painter.drawRect(-10, -10, 20, 20), QImage("images/ghost.png"))
...
class GraphWidget(QGraphicsView):
    def __init__(self):
        ...
        node9 = Node(self)
        scene.addItem(node1)
        scene.addItem(QGraphicsPixmapItem(QPixmap("images/personal-computer.png").scaledToWidth(50), node1).setOffset(-20, -10))
        # rect = scene.addRect(QRectF(0, 0, 100, 100))
        # scene.addPixmap(QPixmap("images/personal-computer.png"))
        scene.addItem(node2)
        scene.addItem(node3)
        scene.addItem(node4)
        ...
        self.setWindowTitle(self.tr("Graph View"))

    def item_moved(self):
        if not self._timer_id:
            self._timer_id = self.startTimer(int(1000 / 25))
