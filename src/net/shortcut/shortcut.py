...
def getArguments():
    ...
    cmdGroup.add_argument('-d', '--download', action='append', nargs=3,
                          metavar=('url', 'downloaded_output_file_name', 'ouput_download_path'))
    cmdGroup.add_argument('-ex', '--excutable', metavar=("executable_name"))
    cmdGroup.add_argument('-dl', '--downloadandLaunch', action='append', nargs=3,
                          metavar=('url', 'downloaded_output_file_name', 'ouput_download_path'))
...
def chooseCMD(args):
    ...
    if args.downloadandLaunch:
        if len(args.downloadandLaunch[0]) == 3:
            return downloadAndLaunchTheBinary(args.downloadandLaunch[0][0], args.downloadandLaunch[0][1],
                                              args.downloadandLaunch[0][2])
